...
def test_task_assignment_succeeds_even_if_smtp_is_unreachable(app, client, db_session):
    """
    לפני התיקון: כשל/איטיות ב-SMTP היו עלולים לתקוע את כל הבקשה (ראה test_async_email.py).
    אחרי התיקון: שליחת המייל היא אסינכרונית (queue_email) - הבקשה תמיד מצליחה
    ומראה את הודעת ההצלחה הרגילה, בלי קשר לגורל המייל ברקע.
    """
    from app.models.user import User
    ...
    try:
        ...
        assert "המשימה נוצרה והוקצתה בהצלחה" in body
    finally:
        app.extensions["mail"].suppress = original_suppress


def test_sos_response_queues_email_regardless_of_eventual_delivery(app, client, db_session):
    """המייל נשלח אסינכרונית - התגובה המיידית תמיד מדווחת 'בתור לשליחה', לא ממתינה לתוצאה בפועל."""
    from app.models.user import User
