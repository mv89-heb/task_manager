...
def validate_password_strength(password):
    """
    בדיקת חוזק סיסמה מינימלית: לפחות 8 תווים, כולל גם אות וגם ספרה.
    לא דורש תווים מיוחדים בכוונה - כדי לא להכביד על משתמשים פחות טכניים.
    מחזיר (True, None) אם תקין, או (False, "הודעת שגיאה בעברית") אם לא.
    """
...
class User(UserMixin, db.Model):
    ...
    # דגל: אם True, המשתמש יופנה חובה למסך שינוי סיסמה בכניסה הבאה שלו.
    # מוגדר אוטומטית ע"י /rescue (סיסמה ידועה "123456"), או ידנית ע"י admin/manager.
    must_change_password = db.Column(db.Boolean, default=False)

    # מתי המשתמש התחבר בפעם האחרונה - מוצג בפאנל הניהול, עוזר לזהות מי בכלל משתמש במערכת
